...
export async function lrcLibFetch(song: Song): Promise<LyricSearchResult[]> {
  ...
  const res = await fetch(`https://lrclib.net/api/get?track_name=${song.title.replaceAll(" ", "+")}&artist_name=${(song.artists || ["undefined"]).join(" ").replaceAll(" ", "+")}&album_name=${(song.album || "undefined").replaceAll(" ", "+")}&duration=${song.duration}`, { headers: {"lrclib-client": `LyricKit/${process.env.IMAGE_TAG || "Unknown"} (https://github.com/aelithron/LyricKit)`} });
  if (!res.ok) return [];
