    ...
    <div className="grid grid-rows-3 grid-cols-1 md:grid-cols-3 md:grid-rows-1 mt-6 gap-4">
      <div className="flex flex-col text-center p-3 gap-2">
        ...
        {missingLyrics?.map((song) => (
          <div key={song.title} className="flex gap-3 bg-slate-500 p-2 rounded-lg">
            <SongCard song={song} />
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col bg-slate-500 rounded-lg text-center p-3 gap-2 h-min">
        ...
        <form className="flex flex-col text-start gap-1">
          <div>
          ...
          <button type="submit" className="bg-violet-300 text-black rounded-lg p-1 w-fit"><FontAwesomeIcon icon={faSearch} /> Search</button>
        </form>
      </div>
    </div>
  )
